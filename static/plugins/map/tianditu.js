...
function initTianditu()
{
	...
	dojo.declare("TDTVecLayer", esri.layers.TiledMapServiceLayer, {
		...
		getTileUrl: function (level, row, col) {
		    if (level <= 14)
		        return "http://t" + serverID + ".tianditu.com/vec_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=vec&TILEMATRIXSET=c&TILEMATRIX=" + level + "&" + "TILEROW=" + row + "&" + "TILECOL=" + col;
		    else
	...
	dojo.declare("TDTImgLayer", esri.layers.TiledMapServiceLayer, {
	    ...
	    getTileUrl: function (level, row, col) {
	        if (level <= 14)
	            return "http://t" + serverID + ".tianditu.com/img_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&TILEMATRIXSET=c&TILEMATRIX=" + level + "&" + "TILEROW=" + row + "&" + "TILECOL=" + col;
	        else
	...
	dojo.declare("TDTEWordLayer", TDTVecLayer, {
		getTileUrl: function(level, row, col) {  
		    return "http://t" + serverID + ".tianditu.com/cva_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=cva&TILEMATRIXSET=c&TILEMATRIX=" + level + "&" + "TILEROW=" + row + "&" + "TILECOL=" + col;
		}
	});
     //卫星图文字标注  
	dojo.declare("TDTImgWordLayer", TDTImgLayer, {
	    getTileUrl: function (level, row, col) {
	        return "http://t" + serverID + ".tianditu.com/cia_c/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=cia&TILEMATRIXSET=c&TILEMATRIX=" + level + "&" + "TILEROW=" + row + "&" + "TILECOL=" + col;
	    }
	});
